...
import { bookings, cars, users, clients, maintenance, payments } from '../db/schema';
...
export const generateTestData = async (options: GenerateOptions = {}) => {
    ...
    if (db) {
        ...
        const carBrands = ['Toyota', 'Honda', 'Ford', 'BMW', 'Mercedes', 'Audi', 'Volkswagen', 'Hyundai', 'Kia', 'Nissan'];
        const carModels = {
        ...
        const colors = ['Black', 'White', 'Silver', 'Gray', 'Red', 'Blue', 'Green', 'Yellow'];
        const locations = ['Downtown', 'Airport', 'City Center', 'Shopping Mall', 'Train Station'];
        ...
        const carData = Array.from({ length: carsCount }, (_, i) => {
            ...
            const category = faker.helpers.arrayElement(['economy', 'comfort', 'business'] as const);

            // Different pricing based on category
            const dailyPrice = category === 'economy' ?
            ...
            return {
                ...
                currentMileage: faker.number.float({ min: 5000, max: 80000, fractionDigits: 0 }),
                status: faker.helpers.arrayElement(['available', 'rented', 'maintenance'] as const),
        ...
        const insertedMaintenance = await db.insert(maintenance).values(maintenanceData).returning();
        console.log(`✅ Generated ${insertedMaintenance.length} maintenance records`);
        ...
        const bookingData = Array.from({ length: bookingsCount }, (_, i) => {
            ...
            const status = faker.helpers.arrayElement(['pending', 'confirmed', 'active', 'completed', 'cancelled'] as const);
            const paymentStatus = status === 'completed' || status === 'active' ?
                'paid' as const :
                status === 'cancelled' ?
        ...
        const paymentData = insertedBookings
            ...
            .map(booking => {
                ...
                return {
                    ...
                    paymentDate: faker.date.between({ from: booking.createdAt, to: new Date() }),
                    createdAt: booking.createdAt
                };
            });

        const insertedPayments = await db.insert(payments).values(paymentData as any).returning();
